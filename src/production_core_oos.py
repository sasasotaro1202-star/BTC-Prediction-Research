...
def _compare(candidate: dict[str, dict], champion_name: str = "frozen_production") -> dict:
    ...
    for name, item in candidate.items():
        # Diagnostic cohorts such as high_disagreement do not expose an
        # overall model score; they must never enter the model comparison map.
        if not isinstance(item, dict) or "overall" not in item:
            continue
