...
INTERVAL=300; FEATURES=['ret_1m','ret_3m','ret_5m','ret_10m','volatility_10m','volume_ratio']
...
def stable_probs(f):
    score=2.2*f['ret_5m']+1.3*f['ret_10m']+0.8*f['ret_3m']; score/=max(0.0015,f['volatility_10m']/2); score=max(-2.0,min(2.0,score))
    up=1/(1+math.exp(-score)); flat=max(0.10,min(0.30,0.22-0.25*abs(f['ret_5m'])/max(0.001,f['volatility_10m']))); up=(1-flat)*up; down=1-flat-up
...
def model_probs(h,f):
    ...
    try:
        model=joblib.load(path); X=np.array([[f[k] for k in FEATURES]],dtype=float); raw=model.predict_proba(X)[0]
...
def predict():
    init_db(); rows=fetch_klines(); f=features(rows); fallback=stable_probs(f)
    now=utcnow(); t5=next_grid(now,1); t10=next_grid(now,2); price=float(rows[-1][4])
    p5=model_probs('5m',f) or fallback; p10=model_probs('10m',f) or fallback
    v5=registry_version('5m'); v10=registry_version('10m')
    scenario={'5m':p5,'10m':p10,'fallback_model':fallback}
    with sqlite3.connect(DB) as con:
