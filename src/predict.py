...
def fallback_probs(f):
    score=2.2*f['ret_5m']+1.3*f['ret_10m']+0.8*f['ret_3m']
    score/=max(0.0015,f['volatility_10m']/2); score=max(-2,min(2,score))
    up=1/(1+math.exp(-score)); flat=max(0.10,min(0.30,0.22-0.25*abs(f['ret_5m'])/max(0.001,f['volatility_10m'])))
    up=(1-flat)*up
    return {'DOWN':1-flat-up,'FLAT':flat,'UP':up}
...
def stabilize(probs, fallback):
    # Blend production model with structural fallback. This prevents small tick changes
    # from causing unstable probability swings while preserving strong model signals.
    p=np.array([probs['DOWN'],probs['FLAT'],probs['UP']],float)
    q=np.array([fallback['DOWN'],fallback['FLAT'],fallback['UP']],float)
    out=0.75*p+0.25*q
    # Avoid overconfident 5m/10m probabilities from a tiny feature set.
...
def predict():
    ...
    f=features(rows); fallback=fallback_probs(f); now=utcnow(); t5=next_grid(now,1); t10=next_grid(now,2); price=float(rows[-1][4])
    p5=stabilize(model_probs(load_production_model('5m'),f) or fallback,fallback)
    p10=stabilize(model_probs(load_production_model('10m'),f) or fallback,fallback)
    v5=registry_version('5m'); v10=registry_version('10m')
    scenario={'5m':p5,'10m':p10,'fallback_model':fallback,'price_source':'coinbase_btc_usd','probability_policy':'stable_blend_v2'}
    with sqlite3.connect(DB) as con:
        con.execute('INSERT INTO predictions(created_at_utc,target_5m,target_10m,base_price,p_up_5m,p_down_5m,p_flat_5m,p_up_10m,p_down_10m,p_flat_10m,model_version,feature_json,scenario_json) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)',(
            now.isoformat(),t5.isoformat(),t10.isoformat(),price,p5['UP'],p5['DOWN'],p5['FLAT'],p10['UP'],p10['DOWN'],p10['FLAT'],f'5m:{v5}|10m:{v10}',json.dumps(f),json.dumps(scenario)))
