...
def logistic_probs(f, horizon):
    path=DB.parent/'models'/f'{horizon}.json'
    if not path.exists(): return None
    try:
        m=json.loads(path.read_text(encoding='utf-8'))
        x=[float(f[k]) for k in m['features']]
        scores=[]
        for coef, intercept in zip(m['coef'], m['intercept']):
            scores.append(sum(a*b for a,b in zip(coef,x))+intercept)
        mx=max(scores); ex=[math.exp(s-mx) for s in scores]; z=sum(ex)
        raw={c:v/z for c,v in zip(m['classes'],ex)}
...
def choose_probs(f, horizon):
    p=logistic_probs(f,horizon)
    return p if p is not None else stable_probs(f)


def predict():
    init_db(); rows=fetch_klines(); f=features(rows)
    p5=choose_probs(f,'5m'); p10=choose_probs(f,'10m')
    ...
    v5=production_version('5m'); v10=production_version('10m')
    ...
    with sqlite3.connect(DB) as con:
        con.execute('INSERT INTO predictions(created_at_utc,target_5m,target_10m,base_price,p_up_5m,p_down_5m,p_flat_5m,p_up_10m,p_down_10m,p_flat_10m,model_version,feature_json,scenario_json) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)',
          (now.isoformat(),t5.isoformat(),t10.isoformat(),price,p5['UP'],p5['DOWN'],p5['FLAT'],p10['UP'],p10['DOWN'],p10['FLAT'],model_version,json.dumps(f),json.dumps(scenario)))
