...
def compact(obj: dict[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(obj.get("final_holdout"), dict):
        out["final_holdout"] = obj["final_holdout"]
    return out


def build() -> dict[str, Any]:
    report, pit, ria, artifact, gate = [
        load(name)
        for name in (
            "report.json",
            "pit_oos_audit.json",
            "research_input_audit.json",
            "production_artifact_audit.json",
            "promotion_gate.json",
        )
    ]
    return {
        ...
        "pit_oos": {
            ...
            for key in (
                ...
                "violation_count",
                "checked_predictions",
                "verified_primary_predictions",
                "min_strict_pit_rows",
            )
        },
        "research_input_audit": {
            key: ria.get(key) for key in ("ok", "schema_version", "data_source")
        },
...
def main() -> None:
    ...
    print(json.dumps({"ok": True, "bytes": OUT.stat().st_size, "path": str(OUT)}))
