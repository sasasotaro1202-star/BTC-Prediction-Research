...
OUT = RESEARCH / "dual_window_calibrated_routing_oos.json"
...
CALIBRATION_TEMPERATURES = tuple(np.round(np.linspace(0.50, 1.80, 27), 2))
...
def _choose_temperature(y: list[str], p: np.ndarray) -> tuple[float, dict[str, float]]:
    best: tuple[float, float] | None = None
    yi = np.asarray([CLASSES.index(v) for v in y], dtype=int)
    base = _norm(p)
    for t in CALIBRATION_TEMPERATURES:
        q = _temperature_scale(base, t)
        ll = float(-np.mean(np.log(np.clip(q[np.arange(len(yi)), yi], EPS, 1.0))))
        key = (ll, abs(float(t) - 1.0))
        if best is None or key < best:
            best = key
    if best is None:
        raise RuntimeError("temperature_search_empty")
    temperature = float(best[1])
    q = _temperature_scale(base, temperature)
    m = _metrics(y, q)
    return temperature, {
...
def evaluate(horizon: str) -> dict[str, Any]:
    ...
    for test_start in range(
        ...
        tune_routed, tune_route_mask = _route(
            tune_prod, tune_alt, tune_short_gains * short_weight + (1.0 - short_weight) * tune_long_gains,
            threshold, alpha,
        )
        tune_route_mask &= np.max(np.abs(tune_short_gains - tune_long_gains), axis=1) <= disagreement_cap
        tune_gated = tune_prod.copy()
        tune_gated[tune_route_mask] = tune_routed[tune_route_mask]
        tune_gated = _norm(tune_gated)
        temperature, _ = _choose_temperature(tune_y, tune_gated)
        candidate = _temperature_scale(candidate, temperature)
    ...
    final_tune_routed, final_tune_mask = _route(
        final_tune_prod,
        {name: probs[name][final_tune_idx] for name in ALTERNATIVES},
        final_short_gains,
        final_threshold,
    ...
    final_tune_mask &= np.max(np.abs(final_short_gains - final_long_gains), axis=1) <= final_disagreement_cap
    final_tune_gated = final_tune_prod.copy()
    final_tune_gated[final_tune_mask] = final_tune_routed[final_tune_mask]
    final_tune_gated = _norm(final_tune_gated)
    final_temperature, final_temperature_metrics = _choose_temperature(
        [y[i] for i in final_tune_idx], final_tune_gated
    )
    hold_candidate = _temperature_scale(hold_candidate, final_temperature)
    ...
    return {
        ...
        "final_holdout": {
            ...
            "temperature_validation": final_temperature_metrics,
