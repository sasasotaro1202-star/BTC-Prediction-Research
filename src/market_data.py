...
UA = "BTC-Prediction-Research/6.0"
...
def coinbase_rows(limit: int = 300):
    end = int(time.time()); start = end - min(limit, 300) * 60
...
def cache_rows(limit=120):
    try:
        ...
        now=int(time.time()*1000); rows=[r for r in rows if r[0]+60000<=now]
        return rows[-limit:], obj.get('created_at_utc','')
    except Exception:
        return [], ''
...
def resilient_1m_series(limit: int = 120):
    ...
    if len(fut) < 40 and len(by) >= 40:
        ...
        try:
            ...
            if len(cb) >= 40:
                fut = cb; status["price_feature_fallback"] = "coinbase"
            else: raise RuntimeError("insufficient Coinbase candles")
        except Exception as e:
            status["coinbase_futures"] = f"error:{type(e).__name__}"
            try:
                kr = kraken_rows(max(120, limit))
                if len(kr) >= 40:
                    fut = kr; status["price_feature_fallback"] = "kraken"
                else: raise RuntimeError("insufficient Kraken candles")
            except Exception as e2:
                status["kraken_futures"] = f"error:{type(e2).__name__}"
                cached, created = cache_rows(limit)
                if len(cached) >= 40:
                    fut = cached; status["price_feature_fallback"] = "bootstrap_cache"; status["cache_created_at_utc"] = created
                else: status["price_feature_fallback"] = "none"
    else:
...
def target_close_binance(target_iso: str):
    ...
    try:
        ...
        for row in rows:
            if int(row[0])==start:return float(row[4]),"binance"
    ...
    try:
        ...
        for row in payload.get("result",{}).get("list",[]):
            if int(row[0])==start:return float(row[4]),"bybit"
    except Exception: pass
    try:
        for row in coinbase_rows(10):
            if row[0]==start:return float(row[4]),"coinbase"
    except Exception: pass
    try:
        for row in kraken_rows(10):
            if row[0]==start:return float(row[4]),"kraken"
    except Exception: pass
