...
def load_rows(horizon):
    ...
    with sqlite3.connect(DB) as con:
        rows = con.execute(
            f"SELECT prediction_id, created_at_utc, feature_json, {actual_col}, "
            f"p_up_{horizon}, p_down_{horizon}, p_flat_{horizon}, model_version "
            "FROM predictions WHERE " + actual_col + " IS NOT NULL ORDER BY created_at_utc"
        ).fetchall()
    out = []
    for r in rows:
        f = safe_json(r[2])
        if not all(k in f for k in FEATURES):
            continue
        if r[3] not in CLASSES:
            continue
        x = [float(f[k]) for k in FEATURES]
        if not all(math.isfinite(v) for v in x):
            continue
        out.append({"id": r[0], "created": r[1], "x": x, "y": r[3],
                    "production": [float(r[4]), float(r[5]), float(r[6])],
                    "model_version": r[7]})
    return out
...
def metrics(y_true, probs):
    ...
    y = np.array([idx[v] for v in y_true])
...
def production_oos(rows):
    return metrics([r["y"] for r in rows], [r["production"] for r in rows])
...
def walk_forward(rows, factory):
    ...
    for end in range(MIN_TRAIN, len(rows), TEST_BLOCK):
        ...
        if len(set(y)) < 2:
            continue
        model.fit(X, y)
        p = model.predict_proba(np.array([r["x"] for r in test]))
        classes = list(model.classes_)
        aligned = np.full((len(test), len(CLASSES)), 1e-6)
        for j, c in enumerate(classes):
            aligned[:, CLASSES.index(c)] = p[:, j]
        aligned /= aligned.sum(axis=1, keepdims=True)
        preds.extend(aligned.tolist())
        ys.extend(r["y"] for r in test)
...
def materially_better(candidate, production):
    return (
        candidate["accuracy"] >= production["accuracy"] - 0.01
        and candidate["logloss"] <= production["logloss"] - 0.005
        and candidate["brier"] <= production["brier"] - 0.002
        and candidate["calibration_error"] <= production["calibration_error"] + 0.01
    )
...
def save_metric(horizon, version, n, m):
    with sqlite3.connect(DB) as con:
        con.execute(
            "INSERT INTO model_metrics(evaluated_at_utc,horizon,model_version,n,accuracy,logloss,brier,calibration_error) VALUES(?,?,?,?,?,?,?,?)",
            (now(), horizon, version, n, m["accuracy"], m["logloss"], m["brier"], m["calibration_error"]),
        )
...
def set_production_version(horizon, version):
    with sqlite3.connect(DB) as con:
        con.execute(
            ...
            "ON CONFLICT(horizon) DO UPDATE SET production_version=excluded.production_version, updated_at_utc=excluded.updated_at_utc",
            (horizon, version, now()),
        )
...
def compare_horizon(horizon):
    ...
    if len(rows) < MIN_SAMPLES:
        print(f"{horizon}: insufficient OOS data ({len(rows)}/{MIN_SAMPLES})")
        return {"status": "insufficient_data", "n": len(rows)}

    production = production_oos(rows)
    save_metric(horizon, get_production_version(horizon), len(rows), production)
    ...
    for name, factory in candidates.items():
        ...
        if oos is None:
            continue
        results[name] = oos
        save_metric(horizon, name, len(rows), oos)

    eligible = [(name, m) for name, m in results.items() if materially_better(m, production)]
    if not eligible:
        print(f"{horizon}: REJECT — no candidate passed adoption gate")
        return {"status": "rejected", "production": production, "candidates": results}
    ...
    if payload is None:
        print(f"{horizon}: REJECT — winner {winner} is not deployable")
        return {"status": "rejected_not_deployable", "winner": winner}

    new_version = f"v2.{datetime.now(timezone.utc).strftime('%Y%m%d%H%M')}"
    payload["model_version"] = new_version
    (MODEL_DIR / f"{horizon}.json").write_text(json.dumps(payload, indent=2), encoding="utf-8")
    set_production_version(horizon, new_version)
    print(f"{horizon}: ADOPT {new_version} from {winner}")
    print("old", production)
    print("new", winner_metrics)
    return {"status": "adopted", "version": new_version, "source": winner, "old": production, "new": winner_metrics}


def compare():
    init_db()
    summary = {h: compare_horizon(h) for h in HORIZONS}
