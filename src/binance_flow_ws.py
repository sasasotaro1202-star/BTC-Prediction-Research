...
def _valid_time(event_ms:int, received_ms:int) -> bool:
    # Strict PIT: exchange event time must not be later than local receipt.
    # Clock skew is handled by explicit DEFERRED behavior, never by admitting future data.
    return event_ms>0 and received_ms>0 and event_ms<=received_ms
