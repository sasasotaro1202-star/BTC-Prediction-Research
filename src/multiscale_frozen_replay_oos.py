...
def _post_champion_cutoff_ms():
    """Use the latest Champion training timestamp as a hard archive replay cutoff."""
    cutoffs = []
    for horizon in ("5m", "10m"):
        path = ROOT / "models" / f"{horizon}.json"
        try:
            meta = json.loads(path.read_text(encoding="utf-8"))
            trained_raw = meta.get("trained_at_utc")
            trained = datetime.fromisoformat(
                str(trained_raw).replace("Z", "+00:00")
            )
            cutoffs.append(int(trained.timestamp() * 1000))
        except (OSError, TypeError, ValueError, json.JSONDecodeError) as exc:
            raise RuntimeError(
                f"missing or invalid Champion training timestamp for {horizon}"
            ) from exc
    return max(cutoffs)
...
def _dataset():
    ...
    required = MIN_TRAIN + MIN_SELECTION + MIN_GATE + FINAL_REPLAY_WINDOWS * REPLAY_MIN_ROWS
    if len(rows) < required:
        return None, {
            "status": "DEFERRED",
            "reason": "insufficient_post_champion_archive_dataset",
            "rows": len(rows),
            "required_rows": required,
            "sample_size_class": "small_post_champion_research",
        }
    return rows, {
        "status": "OK",
        "rows": len(rows),
        "required_rows": required,
        "sample_size_class": "small_post_champion_research" if len(rows) < 16_000 else "standard",
    }
...
def main():
    dataset, dataset_status = _dataset()
    if dataset is None:
        payload = {
            "schema_version": 1,
            "research_only": True,
            ...
            "horizons": {
                "5m": {"status": "DEFERRED", **dataset_status},
                "10m": {"status": "DEFERRED", **dataset_status},
            },
        }
