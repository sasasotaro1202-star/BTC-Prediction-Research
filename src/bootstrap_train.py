...
def make_features(rows):
    c = np.asarray([r[4] for r in rows], float); o = np.asarray([r[1] for r in rows], float); h = np.asarray([r[2] for r in rows], float); l = np.asarray([r[3] for r in rows], float); v = np.asarray([r[5] for r in rows], float)
    p = c[-1]; ret = lambda n: c[-1] / c[-1 - n] - 1
    r1, r3, r5, r10 = [ret(n) for n in (1, 3, 5, 10)]
    accel = r1 - r3 / 3; rv5 = float(np.std(np.diff(c[-6:]) / c[-6:-1])); rv10 = float(np.std(np.diff(c[-11:]) / c[-11:-1]))
    hi, lo = max(h[-10:]), min(l[-10:]); rp = (p - lo) / (hi - lo) if hi > lo else 0.5
    body = (p - o[-1]) / p; up = (h[-1] - max(o[-1], p)) / p; low = (min(o[-1], p) - l[-1]) / p
    old = float(np.mean(v[-15:-5])); rvol = float(np.mean(v[-5:])) / max(1e-12, old); vtrend = float(np.mean(v[-5:])) / max(1e-12, float(np.mean(v[-10:])))
...
def publish(horizon, best, baseline, holdout_n):
    ...
    MODEL_DIR.mkdir(parents=True, exist_ok=True); joblib.dump(model, MODEL_DIR / f"{horizon}.joblib"); version = f"bootstrap.{name}.v5"
    meta = {"model_version": version, "horizon": horizon, "classes": list(model.classes_), "features": FEATURES, "artifact": f"{horizon}.joblib", "candidate": False, "bootstrap": True, "holdout_n": holdout_n, "holdout_metrics": score, "baseline_metrics": baseline, "temperature": float(temperature), "trained_at_utc": datetime.now(timezone.utc).isoformat()}
    (MODEL_DIR / f"{horizon}.json").write_text(json.dumps(meta, indent=2), encoding="utf-8")
    with sqlite3.connect(DB) as con:
        con.execute("INSERT INTO model_registry(horizon,production_version,updated_at_utc) VALUES(?,?,?) ON CONFLICT(horizon) DO UPDATE SET production_version=excluded.production_version,updated_at_utc=excluded.updated_at_utc", (horizon, version, datetime.now(timezone.utc).isoformat()))
    return True, meta
...
def main():
    ...
    try:
        ...
        write_status({"status": "history_failed", "error": f"{type(exc).__name__}: {exc}"}); return 0
    if len(rows) < MIN_BOOTSTRAP_ROWS:
        write_status({"status": "insufficient_history", "rows": len(rows), "minimum": MIN_BOOTSTRAP_ROWS, "source": source}); return 0
    ...
    for horizon in ("5m", "10m"):
        ...
        if len(y) < MIN_TRAIN + MIN_OOS or len(set(y)) < 3:
            published.append({"horizon": horizon, "status": "insufficient_dataset", "rows": len(y)}); continue
        best, baseline, holdout_n = train_one(X, y); ok, meta = publish(horizon, best, baseline, holdout_n)
