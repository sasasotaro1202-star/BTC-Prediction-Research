...
Design goals:
- historical-data outages never block live inference;
- free/public sources are tried in a deterministic order;
- static Binance Vision archives are preferred for deep bootstrap history;
- production models are replaced only after a chronological holdout gate;
- insufficient history is a normal, nonfatal state.
"""
...
def write_status(payload: dict) -> None:
    ...
    STATUS.write_text(
        json.dumps({**payload, "updated_at_utc": datetime.now(timezone.utc).isoformat()}, indent=2),
        encoding="utf-8",
    )
...
def _bybit_page(end_ms: int | None = None, limit: int = 1000):
...
def _binance_page(end_ms: int | None = None, limit: int = 1500):
...
def fetch_bybit(target: int):
    ...
    for _ in range(math.ceil(target / 1000) + 8):
        payload = _bybit_page(end, 1000)
        raw = payload.get("result", {}).get("list", []) if isinstance(payload, dict) else []
        if not raw:
            break
        for r in raw:
            if len(r) >= 6:
...
def make_features(rows):
    ...
    r1, r3, r5, r10 = [ret(n) for n in (1, 3, 5, 10)]
    accel = r1 - r3 / 3
    rv5 = float(np.std(np.diff(c[-6:]) / c[-6:-1]))
    rv10 = float(np.std(np.diff(c[-11:]) / c[-11:-1]))
    hi, lo = max(h[-10:]), min(l[-10:])
    rp = (p - lo) / (hi - lo) if hi > lo else 0.5
    body = (p - o[-1]) / p
    up = (h[-1] - max(o[-1], p)) / p
    low = (min(o[-1], p) - l[-1]) / p
    old = float(np.mean(v[-15:-5]))
    rvol = float(np.mean(v[-5:])) / max(1e-12, old)
    vtrend = float(np.mean(v[-5:])) / max(1e-12, float(np.mean(v[-10:])))
    return [
        r1, r3, r5, r10, accel, rv5, rv10, rp, body, up, low, rvol,
        vtrend, p / ema(c[-20:], 5) - 1, p / ema(c[-30:], 10) - 1,
    ]
...
def build_dataset(rows, horizon):
    ...
    for i in range(30, len(rows) - horizon):
        ...
        label = "UP" if future_return > THRESHOLD else "DOWN" if future_return < -THRESHOLD else "FLAT"
...
def normalize(probs):
    p = np.clip(np.asarray(probs, float), 1e-8, 1)
    return p / p.sum(axis=1, keepdims=True)


def metrics(y, probs):
    idx = {c: i for i, c in enumerate(CLASSES)}
    yi = np.asarray([idx[z] for z in y])
    p = normalize(probs)
    one = np.eye(3)[yi]
    return {
...
def fit_temperature(probs, y):
    if len(y) < 200 or len(set(y)) < 3:
        return 1.0
    p = normalize(probs)
    yi = np.asarray([{c: i for i, c in enumerate(CLASSES)}[z] for z in y])
    split = max(int(len(y) * 0.75), 100)
    logits = np.log(np.clip(p, 1e-8, 1))
    best_t, best_loss = 1.0, float("inf")
    for t in np.linspace(0.7, 2.5, 73):
        z = logits[:split] / t
        z -= z.max(axis=1, keepdims=True)
        q = np.exp(z); q /= q.sum(axis=1, keepdims=True)
        ll = log_loss(yi[:split], q, labels=[0, 1, 2])
        if ll < best_loss:
            best_loss, best_t = ll, float(t)
    z = logits[split:] / best_t
    z -= z.max(axis=1, keepdims=True)
    q = np.exp(z); q /= q.sum(axis=1, keepdims=True)
    raw = log_loss(yi[split:], normalize(p[split:]), labels=[0, 1, 2])
    scaled = log_loss(yi[split:], q, labels=[0, 1, 2])
    return best_t if scaled < raw - 0.001 else 1.0
...
def train_one(X, y):
    ...
    baseline_probs = np.tile(np.asarray([np.mean(ytr == c) for c in CLASSES]), (len(yte), 1))
    baseline = metrics(yte, baseline_probs)
    candidates = [
        ("logreg", Pipeline([("scale", StandardScaler()), ("model", LogisticRegression(C=0.5, max_iter=3000))])),
        ("rf", RandomForestClassifier(n_estimators=300, max_depth=7, min_samples_leaf=12, max_features="sqrt", random_state=42, n_jobs=-1)),
        ("hgb", HistGradientBoostingClassifier(max_iter=220, max_leaf_nodes=15, learning_rate=0.04, l2_regularization=1.5, random_state=42)),
    ]
    results = []
    for name, model in candidates:
        model.fit(Xtr, ytr)
        temperature = fit_temperature(model.predict_proba(Xcal), ycal)
        p = normalize(model.predict_proba(Xte))
        if temperature != 1.0:
            z = np.log(p) / temperature; z -= z.max(axis=1, keepdims=True); p = np.exp(z); p /= p.sum(axis=1, keepdims=True)
        score = metrics(yte, p)
        results.append((score["logloss"], score["brier"], -score["accuracy"], name, model, temperature, score))
    results.sort(key=lambda r: r[:3])
    return results[0], baseline, len(yte)
...
def publish(horizon, best, baseline, holdout_n):
    ...
    with sqlite3.connect(DB) as con:
        con.execute(
            "INSERT INTO model_registry(horizon,production_version,updated_at_utc) VALUES(?,?,?) ON CONFLICT(horizon) DO UPDATE SET production_version=excluded.production_version,updated_at_utc=excluded.updated_at_utc",
